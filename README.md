...
- 外注費ROI評価
- ガバナンス遵守確認
- **freee試算表の取得**（`skills/freee_fetch.py` で PL・BS を local-workspace/input/ に保存）
...
業務エージェントとは独立したモジュールとして、`Agent-team/Agent-Aiko/` に配置された **人格レイヤー**。CLAUDE.md 単独でフル動作するよう設計され、別端末や派生プロジェクトへもポータブルに展開できる。

### モード

| モード | 説明 |
|---|---|
| `origin` | リポジトリ管理者が定義したデフォルト人格。書き換え不可。 |
| `override` | ユーザーがカスタマイズした人格。INVARIANTS.md の不変条項は超えられない。 |
...
- **配置**：2026-05-01 のリファクタで業務エージェント (`agents/`) から分離。`Agent-team/Agent-Aiko/` 配下で独立に起動
