...
エージェント間の協調や外部 LLM への委譲に使われる、すべてのエージェントから利用できる共通スキル。いずれも subprocess 起動 + 1 ターン非対話の delegation パターン。

### `/agent-call` — Claude → Claude（別人格）への委譲

別エージェントの `.claude/CLAUDE.md`・skills・hooks を読み込んだ状態で 1 ターン質問する。`claude --print` を target agent ディレクトリで起動するシンプルな構造。

| コマンド | 動作 |
|---|---|
| `/agent-call list` | 利用可能なエージェント一覧 |
| `/agent-call status` | claude CLI と agents/ の状態 |
| `/agent-call <agent-name> "<prompt>"` | 指定エージェントへ 1 ターン委譲 |

### `/codex` — Claude → ChatGPT (Codex App Server) への委譲

ChatGPT の Codex App Server に問い合わせて、別モデルの視点を取り込む。第二意見・並列調査・コード生成の handoff に使う。

| コマンド | 動作 |
|---|---|
| `/codex setup` | Codex CLI 初回セットアップ |
| `/codex ask <prompt>` | 1 ターン質問 |
| `/codex status` | 認証状態 |

### 設計

- いずれも **同期 1 ターンのみ**（履歴・対話継続なし）
- 呼び出し先のセッション履歴は残さない（`--no-session-persistence` 等）
- 並列起動は rate limit に注意、基本は逐次
- 再帰的な委譲（呼び出し先がさらに同種スキルを呼ぶ）は禁止、1 段まで

---

## 共通ユーティリティ

エージェントから利用できる、外部リソースを取り込む小さな道具類。委譲ではなく取得・抽出・整形の役割。

### `/gslides-read` — Google Slides の本文＋スピーカーノート取得

Google Slides ファイルを Drive API 経由で `.pptx` としてダウンロードし、各スライドの本文とスピーカーノートを Markdown で取り出す。Drive のテキストエクスポートだとスピーカーノートが落ちるため、その補完用。

| コマンド | 動作 |
|---|---|
| `/gslides-read setup` | 依存インストール + 認証確認 |
| `/gslides-read <file-id-or-url>` | 標準出力に Markdown を出力 |
| `/gslides-read <file-id-or-url> -o <path>` | 指定先に書き出し |
...
実装は `python-pptx`、認証は Drive API の Application Default Credentials。Workspace 環境での認証セットアップに制約があり、セルフホスト OAuth クライアントへの切替などが残課題。
...
| `/aiko-tasks` | 残課題（`dev-logs.md` の `### 残課題`）を一覧表示。起動時に件数のみ自動表示。 |
